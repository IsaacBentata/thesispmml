...
class StockTrader():
    ...
    def write(self,codes,agent):
        ...
        history.to_csv(PATH_prefix+agent + '-'.join(str(codes)) + '-' + str(math.exp(np.sum(self.r_history)) * 100) + '.csv')
...
def backtest(agent,env):
    ...
    labels=['PG','Follow the Winner','UCRP','Follow the Loser']
    ...
    print('Asset Name','   ','Return','   ','Sharpe','   ','Max Drawdown')
    plt.figure(figsize=(8, 6), dpi=100)
    for i in range(len(agents)):
        plt.plot(wealths_result[i],label=labels[i])
        mrr=float(np.mean(rs_result[i])*10000)
...
def session(config,args):
    ...
    if args['mode']=='train':
        if not os.path.exists(PATH_prefix):
            ...
            print(start_date)
            train_start_date, train_end_date, test_start_date, test_end_date, codes = env.get_repo(train_start_date, train_end_date,
                                                                                                   codes, market)
