...
def elastic_net(month, features):
    ...
    net = ElasticNet()
    net_model = net.fit(x_train, label_train)

    predicted_y = net_model.predict(x_test)

    return label_test, predicted_y

number = 0

final_elastic = np.ones((967))

true_elastic = np.ones((967))
...
for d in column_names:
    ...
    true, prediction = elastic_net(label, features_dataset)

    final_elastic = np.vstack((final_elastic, prediction))

    true_elastic = np.vstack((true_elastic, true))

final_elastic = np.transpose(final_elastic)
true_elastic = np.transpose(true_elastic)

elastic_trader = short_trader(final_elastic[:600], true_elastic[:600], param1, "price", inven_r[:600], param2)
