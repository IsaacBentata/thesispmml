    ...
    def build_net(self):
        ...
        if network_req == "CNN":
                ...
                with jit_scope():
                    ...
                    network = tf.layers.conv2d(state, 2,
                                                        [1, 2],
                                                        strides=(1, 1),
                                                        padding = 'valid',
                                                        activation = 'relu')
                    width = network.get_shape()[2]
                    network = tf.layers.conv2d(network, 48,
                                                        [1, width],
                                                        strides=(1, 1),
                                                        padding = "valid",
                                                        activation = 'relu')
                    ...
                    network = tf.layers.conv2d(network, 1,
        ...
        if network_req == "NN":
            ...
            print(state.shape, "shape of state")
            network = tf.layers.dense(inputs = state, units = 20, activation = 'relu')
            # width = network.get_shape()[2]
            ...
            w_previous=tf.placeholder(tf.float32,shape=[None,self.M])
            network=tf.concat([network,tf.reshape(w_previous, [-1, self.M, 1, 1])],axis=3)
            network = tf.layers.dense(inputs = network, units = 10, activation = 'relu')
            network=tf.layers.flatten(network)
            w_init = tf.random_uniform_initializer(-0.005, 0.005)
            out = tf.layers.dense(network, self.M, activation=tf.nn.softmax, kernel_initializer=w_init)
