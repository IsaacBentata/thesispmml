...
def lstm(month, features, true_price, d):
  ...
  model_lstm.add(LSTM(20, input_shape=(x_train.shape[1], 1), activation='relu', return_sequences=True))
  model_lstm.add(Dropout(0.2))
  model_lstm.add(LSTM(10, input_shape=(x_train.shape[1], 1), activation='relu', return_sequences=False))
  model_lstm.add(Dropout(0.2))
  ...
  model_lstm.fit(x_train, label_train, epochs = 1000, batch_size = 32, verbose = 0)
...
final_lstm = np.ones((967))

true_lstm = np.ones((967))
...
for d in column_names:
    ...
    true, prediction = lstm(label, features_dataset, label2, d + 'new22')

    final_lstm = np.vstack((final_lstm, prediction))

    true_lstm = np.vstack((true_lstm, true))

final_lstm = np.transpose(final_lstm)
true_lstm = np.transpose(true_lstm)

lstm_trader = short_trader(final_lstm[:600], true_lstm[:600], param1, "price", inven_r[:600], param2)
